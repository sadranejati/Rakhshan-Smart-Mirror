...
def recognizer(phrase_time_limit=0):
    ...
    with sr.Microphone() as source:
        ...
        labelStat.config(text='Listening')
        ...
        labelStat.config(text='Stop Listening')
    try:
        labelStat.config(text='Recognizing')
        query = r.recognize_google(audio, language ='fa-IR', show_all=False)
        labelStat.config(text='Stop Recognizing')
...
img2 = ImageTk.PhotoImage(imageFile2.resize((250, int(imageFile2.size[1]*250/imageFile2.size[0]))))
label1 = Label(frmPrompt, image = img1)
#label1.place(relx=0.6, rely=1, anchor='sw')
label2 = Label(frmPrompt, image = img2)
#label2.place(relx=0.4, rely=1,anchor='se')        
...
def speechrecogonizer():    
    ...
    while True:
        labelStat.config(text='Next iter')
        query = recognizer()
        if type(query) == tuple:
            if state==False:        
            ...
            if('کتاب' in query): 
                ...
                videoplayer.load(r"clip.mp4")
                videoplayer.place(relx=0.5, rely=0.7, anchor=CENTER,width=654,height=360)

                videoplayer.play() # play the video
                mixer.init() #Initialzing pyamge mixer
                mixer.music.load('only_audio.mp3') #Loading Music File
                ...
                mirrorPrompt('چه نوع آهنگی دوست داری؟ پاپ؟ کلاسیک؟ سنتی؟')
                query = recognizer()
                if 'کلاسیک' in query:
                    mirrorPrompt('از گوش دادن به آهنگ بهار از آنتونیو ویوالدی لذت ببر')
                    mixer.init() #Initialzing pyamge mixer
                    mixer.music.load('vivaldi_spring.mp3') #Loading Music File
                    mixer.music.play() #Playing Music with Pygame
                    ...
                    mixer.init() #Initialzing pyamge mixer
                    mixer.music.load('VatanHomayoun.mp3') #Loading Music File
                    mixer.music.play() #Playing Music with Pygame
